  ...
  return (
    <div className="min-h-screen bg-[#F3EDE7] text-[#2B2D42] font-sans flex flex-col">
      ...
      <div className="bg-[#1A1B2E] px-6 py-1.5 flex justify-end gap-5">
        <a href="#/programs" className="text-white/55 text-[11px] font-medium hover:text-white transition-colors">All Programs</a>
        <a href="#/faqs" className="text-white/55 text-[11px] font-medium hover:text-white transition-colors">FAQs</a>
        <a href="#/help" className="text-white/55 text-[11px] font-medium hover:text-white transition-colors">Help Center</a>
      </div>
      ...
      <nav className="bg-white border-b border-[#E5DDD4] px-6 h-[60px] flex items-center justify-between sticky top-0 z-50 shadow-[0_2px_8px_rgba(43,45,66,.06)]">
        <div className="flex items-center gap-6">
          ...
          <div className={`md:flex items-center gap-4 ${navOpen ? 'flex absolute top-[60px] left-0 right-0 bg-white border-b border-[#E5DDD4] p-4 flex-col shadow-lg' : 'hidden md:flex'}`}>
            <a href="#/programs" className="text-[13px] font-medium text-[#6B6D7B] hover:text-[#E87A2E] hover:bg-[#E87A2E]/10 py-1.5 px-2.5 rounded-md transition-colors">Programs</a>
            <a href="#/" className="text-[13px] font-semibold text-[#E87A2E] py-1.5 px-2.5 rounded-md text-center inline-block">Dashboard</a>
            <a href="#myCourses" className="text-[13px] font-medium text-[#6B6D7B] hover:text-[#E87A2E] hover:bg-[#E87A2E]/10 py-1.5 px-2.5 rounded-md transition-colors">My Courses</a>
            <a href="#browse" className="text-[13px] font-medium text-[#6B6D7B] hover:text-[#E87A2E] hover:bg-[#E87A2E]/10 py-1.5 px-2.5 rounded-md transition-colors">Browse</a>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-[13px] font-semibold text-[#2B2D42] hidden sm:block">{user?.name || 'Learner'}</span>
          <div className="w-[34px] h-[34px] rounded-full bg-[#E87A2E]/10 flex items-center justify-center text-[12px] font-bold text-[#E87A2E] cursor-pointer" onClick={onLogout} title="Click to Sign Out">
            {user?.name?.[0].toUpperCase() || 'L'}
          </div>
          <button className="md:hidden p-1 flex flex-col gap-1 cursor-pointer bg-transparent border-none" onClick={() => setNavOpen(!navOpen)}>
      ...
      <main className="max-w-[1060px] w-full mx-auto px-6 py-8 pb-16 flex-1">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5 mb-8">
          {typedDashboardData.stats.map(stat => (
            <div key={stat.id} className="bg-white rounded-[14px] border border-[#E5DDD4] p-4 flex items-center gap-3.5 hover:shadow-[0_2px_8px_rgba(43,45,66,.06)] transition-shadow">
              <div
              ...
              <div>
                 <div className="font-serif text-[1.3rem] text-[#2B2D42]">{stat.value}</div>
                 <div className="text-[11px] text-[#9597A6] font-medium">{stat.label}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Completed Courses */}
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-serif text-[1.15rem] text-[#2B2D42]">Completed Courses</h2>
        ...
        {typedDashboardData.completedCourses.length === 0 ? (
          <div className="bg-white rounded-[14px] border border-[#E5DDD4] p-8 text-center mb-10">
            ...
            <p className="text-[13px] text-[#9597A6]">No completed courses yet. Keep going — your first certificate is within reach!</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
        ...
        <div id="browse">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
             {typedDashboardData.availableCourses.map(course => (
               <div key={course.id} className="bg-white rounded-[14px] border border-[#E5DDD4] overflow-hidden flex flex-col hover:shadow-[0_8px_24px_rgba(43,45,66,.08)] hover:-translate-y-0.5 transition-all">
                 <div className="h-2 w-full" style={{ background: course.bannerGradient }}></div>
                 <div className="p-[1.2rem] flex-1 flex flex-col relative">
                    ...
                    <div className="font-serif text-base text-[#2B2D42] mb-1 leading-[1.3]">{course.title}</div>
                    <div className="text-[12px] text-[#6B6D7B] leading-[1.55] flex-1 mb-3">{course.description}</div>
                    <div className="flex gap-3 mb-3 flex-wrap">
                      {course.meta.map((m, i) => (
                        <span key={i} className="text-[10.5px] text-[#9597A6] font-medium flex items-center gap-1">{m}</span>
                    ...
                    <div className="flex flex-col gap-1 mb-3 border-t border-[#E0E0E0] pt-3">
                       {course.features?.map((feat, i) => (
                         <div key={i} className="text-[11.2px] text-[#6B6D7B] flex items-center gap-[5px]">
                            <svg viewBox="0 0 12 12" fill="none" className="w-[12px] h-[12px] text-[#4CAF50] shrink-0">
                               <path d="M2 6l3 3L10 3" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
                            </svg>
                            {feat}
                         </div>
                       ))}
        ...
        <div className="flex items-center justify-between mb-4 mt-2">
          <h2 className="font-serif text-[1.15rem] text-[#2B2D42]">Recent Activity</h2>
        </div>
        <div className="bg-white rounded-[14px] border border-[#E5DDD4] overflow-hidden mb-8">
           {typedDashboardData.activities.map((act) => (
             <div key={act.id} className="flex items-start gap-3 p-3.5 hover:bg-[#F9F5F0] transition-colors border-b last:border-b-0 border-[rgba(0,0,0,.04)]">
                <div 
                ...
                <div className="flex-1 mt-0.5">
                   <h4 className="text-[12.5px] font-semibold text-[#2B2D42] mb-[2px]">{act.title}</h4>
                   <p className="text-[11.2px] text-[#6B6D7B] leading-[1.45]">{act.description}</p>
                </div>
      ...
      <footer className="bg-[#1A1B2E] p-6 text-center shrink-0">
         <p className="text-[11.5px] text-white/40 mb-0">
           ...
           <a href="#/programs" className="text-[#E87A2E] ml-2 hover:underline no-underline">Programs</a>
           <a href="#/" className="text-[#E87A2E] ml-2 hover:underline no-underline">Terms</a>
           <a href="#/" className="text-[#E87A2E] ml-2 hover:underline no-underline">Privacy</a>
         </p>
      </footer>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/40 z-[100] flex items-center justify-center p-4">
           <div className="bg-white rounded-[14px] p-8 max-w-[400px] w-full text-center shadow-[0_16px_48px_rgba(43,45,66,.12)] relative">
              ...
              <h3 className="font-serif text-[1.2rem] text-[#2B2D42] mb-1.5">Subscribe to {activeCourse?.name}</h3>
              <p className="text-[13px] text-[#6B6D7B] mb-6 leading-[1.5]">
                 You're about to enroll in this course. Once subscribed, it will appear in your dashboard and you can start learning immediately.
              </p>
              <button 
              ...
              <p className="text-[11px] text-[#9597A6] mt-3 mb-0">Free during Summer 2026 pilot program</p>
           </div>
        </div>
      )}
    </div>
