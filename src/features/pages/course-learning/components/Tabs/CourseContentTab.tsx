...
import { UploadCloud, CheckCircle2, ChevronRight, FileText, Video, PenSquare, MessageSquare, Briefcase } from 'lucide-react';
...
export const CourseContentTab: React.FC<CourseContentTabProps> = ({ curW, setCurW, done, markDone }) => {
      ...
      <div className="max-w-[860px] mx-auto p-[1rem] md:p-[1.3rem_1.8rem_3rem]">
        ...
        <div className="bg-white rounded-[14px] border border-[#E5DDD4] mb-[1rem]">
          <div className="p-[2rem] text-center text-[#9597A6]">
            ...
            <h3 className="font-['DM_Serif_Display'] text-[1.17em] text-[#2B2D42] mb-[0.2rem] mt-0">Week Locked</h3>
            <p className="text-[0.78rem] m-0">Complete the previous week to unlock.</p>
          </div>
        </div>
      </div>
    );
  ...
  const postDiscussion = () => {
    ...
    const newMsg = { n: 'You', a: 'Y', tm: 'Now', tx: localMessage.trim(), lk: 0 };
    setDiscussions(prev => ({
      ...prev,
  ...
  const handleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploads(prev => ({ ...prev, [sub.id]: [...(prev[sub.id] || []), ...fileNames] }));
  };
  ...
  return (
    <div className="max-w-[860px] mx-auto p-[1rem] md:p-[1.3rem_1.8rem_3rem]">
      ...
      <div className="font-['DM_Serif_Display'] text-[1.3rem] text-[#2B2D42] mb-[1.2rem]">{w.t}</div>
          ...
          return (
            <div 
              ...
              className={`flex items-center gap-[0.45rem] p-[0.45rem_0.6rem] rounded-[8px] cursor-pointer text-[0.73rem] font-medium transition-all mb-[0.15rem] ${
                a ? 'bg-[#e87a2e1f] text-[#E87A2E] font-semibold' : 
                d ? 'text-[#4CAF50] hover:bg-[#F9F5F0]' : 
                l ? 'opacity-35 cursor-default hover:bg-transparent text-[#6B6D7B]' : 
                'text-[#6B6D7B] hover:bg-[#F9F5F0] hover:text-[#2B2D42]'
              }`}
              onClick={() => { if (!l) setCurS(si); }}
            >
              <div className={`w-[7px] h-[7px] rounded-full border-[1.5px] shrink-0 ${
                a ? 'bg-[#E87A2E] border-[#E87A2E]' : 
                d ? 'bg-[#4CAF50] border-[#4CAF50]' : 
                'border-[#E5DDD4] bg-transparent'
              }`}></div>
              <Icon size={13} className="opacity-40 shrink-0" />
              <div className="flex-1 overflow-hidden text-ellipsis whitespace-nowrap">{s.title}</div>
      ...
      <div className="bg-white rounded-[14px] border border-[#E5DDD4] p-[0.7rem_1rem] mb-[1.2rem] flex items-center gap-[0.8rem]">
        <span className="text-[0.74rem] font-semibold text-[#2B2D42] whitespace-nowrap">{doneCount} of {w.subs.length} completed</span>
        <div className="flex-1 h-[6px] bg-[#E5DDD4] rounded-[3px] overflow-hidden">
          <div className="h-full rounded-[3px] bg-gradient-to-r from-[#E87A2E] to-[#4CAF50] transition-all duration-400" style={{ width: `${pct}%` }}></div>
        </div>
        <span className="text-[0.7rem] font-bold text-[#E87A2E] whitespace-nowrap">{pct}%</span>
      </div>
      ...
      {lk ? (
        <div className="bg-white rounded-[14px] border border-[#E5DDD4] mb-[1rem]">
          <div className="p-[2rem] text-center text-[#9597A6]">
            ...
            <p className="text-[0.78rem] m-0">Complete the previous sub-topic to unlock this one.</p>
          </div>
        </div>
      ) : (
        <>
          {sub.type === 'reading' && (
            <div className="bg-white rounded-[14px] border border-[#E5DDD4] overflow-hidden mb-[1rem]">
              <div className="p-[0.5rem_0.8rem] bg-[#F9F5F0] border-b border-[#E5DDD4] flex items-center justify-between">
                <span className="text-[0.7rem] font-semibold text-[#6B6D7B] flex items-center gap-1.5"><FileText size={14}/> {sub.title}</span>
                <button 
          ...
          {sub.type === 'assess' && sub.questions && (
            ['critical', 'technical', 'problem', 'subjective'].map(catKey => {
              const qs = sub.questions ? sub.questions[catKey as keyof typeof sub.questions] : [];
              if (!qs || qs.length === 0) return null;
              
              const catData = {
                critical: { label: 'Critical Thinking', icon: '🧠', bg: 'rgba(156,39,176,.08)', c: '#9C27B0' },
                technical: { label: 'Technical Depth', icon: '⚙️', bg: 'rgba(66,133,244,.1)', c: '#4285F4' },
                problem: { label: 'Problem Solving', icon: '🧩', bg: '#e87a2e1f', c: '#E87A2E' },
                subjective: { label: 'Subjective', icon: '✍️', bg: '#E8F5E9', c: '#4CAF50' }
              }[catKey];

              return (
                <div key={catKey} className="bg-white rounded-[14px] border border-[#E5DDD4] mb-[0.8rem] overflow-hidden">
                  <div className="p-[0.7rem_1rem] border-b border-[#E5DDD4] flex items-center gap-[0.4rem]">
                    <div className="w-[22px] h-[22px] rounded-[5px] flex items-center justify-center shrink-0 text-[0.75rem]" style={{ backgroundColor: catData?.bg, color: catData?.c }}>
                      {catData?.icon}
                    </div>
                    <h4 className="text-[0.8rem] font-bold text-[#2B2D42] m-0">{catData?.label}</h4>
                    <div className="ml-auto text-[0.6rem] font-semibold text-[#9597A6]">{qs.length}Q</div>
                  </div>
                  <div className="p-[0.9rem_1rem]">
                    {qs.map((q, qi) => {
                      ...
                      return (
                        <div key={qi} className="mb-[1rem] pb-[0.8rem] border-b border-black/5 last:border-none last:mb-0 last:pb-0">
                          <div className="text-[0.63rem] font-bold text-[#E87A2E] uppercase tracking-[0.05em] mb-[0.25rem]">
                            {catData?.label} · Q{qi + 1}
                          </div>
                          <div className="text-[0.82rem] text-[#2B2D42] font-semibold leading-[1.5] mb-[0.5rem]">{q.q}</div>
                          
                          {isMcq ? (
                            <div className="flex flex-col gap-[0.25rem]">
                              {q.opts?.map((o, oi) => {
                                let cls = "flex items-start gap-[0.4rem] p-[0.45rem_0.6rem] rounded-[8px] border-[1.5px] cursor-pointer text-[0.78rem] transition-all ";
                                if (sel !== undefined) {
                                  if (oi === sel && oi === q.ans) cls += "border-[#4CAF50] bg-[#E8F5E9] text-[#2E7D32]";
                                  else if (oi === sel) cls += "border-[#EA4335] bg-[#ea433514] text-[#C62828]";
                                  else if (oi === q.ans) cls += "border-[#4CAF50] bg-[#E8F5E9] text-[#2E7D32]";
                                  else cls += "border-[#E5DDD4] text-[#6B6D7B]";
                                } else {
                                  cls += "border-[#E5DDD4] text-[#6B6D7B] hover:border-[#E87A2E] hover:text-[#E87A2E] hover:bg-[#e87a2e1f]";
                                }

                                return (
                                  <div key={oi} className={cls} onClick={() => { if(sel === undefined) setAnswers(p => ({ ...p, [qid]: oi })) }}>
                                    <div className="w-[15px] h-[15px] rounded-full border-2 shrink-0 mt-[1px] flex items-center justify-center border-current">
                                      {(sel !== undefined && (oi === sel || oi === q.ans)) && (
                              ...
                              {sel !== undefined && (
                                <div className={`p-[0.4rem_0.6rem] rounded-[8px] text-[0.74rem] mt-[0.3rem] leading-[1.4] ${sel === q.ans ? 'bg-[#E8F5E9] text-[#2E7D32]' : 'bg-[#ea433514] text-[#C62828]'}`}>
                                  {sel === q.ans ? 'Correct!' : 'Review the reading material.'}
                                </div>
                              )}
                            </div>
                          ) : (
                            <div>
                              ...
                              {!submitted && (
                                <button 
                                  ...
                                  onClick={() => setAnswers(p => ({ ...p, [`${qid}_sub`]: 1 }))}
                                >
                                  Submit
                                </button>
                              )}
          ...
          {sub.type === 'discussion' && (
            <div className="mb-[1rem]">
              <div className="bg-[#1A1B2E] rounded-t-[14px] p-[0.9rem_1.1rem] text-white">
                <h4 className="font-['DM_Serif_Display'] text-[0.88rem] mb-[0.1rem] m-0">{sub.topic}</h4>
                <p className="text-[0.68rem] text-white/50 m-0">Cohort Alpha-3 · {w.t}</p>
              </div>
              <div className="bg-white border text-[#2B2D42] border-t-0 border-[#E5DDD4] rounded-b-[14px] p-[0.8rem]">
                ...
                {(discussions[sub.id] || sub.seeds || []).map((m: any, i: number) => (
                  <div key={i} className="flex gap-[0.4rem] py-[0.4rem] border-b border-black/5 last:border-none">
                    <div className={`w-[26px] h-[26px] rounded-full flex items-center justify-center text-[0.55rem] font-bold shrink-0 ${i % 2 ? 'bg-[#E8F5E9] text-[#4CAF50]' : 'bg-[#e87a2e1f] text-[#E87A2E]'}`}>
                      {m.a}
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center gap-[0.25rem] mb-[0.08rem]">
                        <span className="text-[0.7rem] font-bold text-[#2B2D42]">{m.n}</span>
                        <span className="text-[0.58rem] text-[#9597A6]">{m.tm}</span>
                      </div>
                      <div className="text-[0.74rem] text-[#6B6D7B] leading-[1.5]">{m.tx}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {sub.type === 'project' && (
            <div className="bg-white rounded-[14px] border border-[#E5DDD4] p-[1.1rem] mb-[1rem]">
              <h4 className="font-['DM_Serif_Display'] text-[0.9rem] text-[#2B2D42] mb-[0.3rem] m-0">{sub.title}</h4>
              <p className="text-[0.78rem] text-[#6B6D7B] leading-[1.6] mb-[0.4rem]">{sub.brief}</p>
              <ul className="m-0 ml-[1rem] p-0 mb-[0.6rem] text-[0.76rem] text-[#6B6D7B] leading-[1.5]">
                {sub.reqs?.map((r, i) => <li key={i} className="mb-[0.15rem]">{r}</li>)}
              </ul>
              
              <label htmlFor={`fu${sub.id}`} className="border-2 border-dashed border-[#E5DDD4] rounded-[14px] p-[1.3rem] text-center cursor-pointer transition-all mb-[0.4rem] block hover:border-[#E87A2E] hover:bg-[#e87a2e1f]">
                <UploadCloud size={22} className="text-[#9597A6] mx-auto mb-[0.2rem] opacity-70" />
                <p className="text-[0.72rem] text-[#9597A6] m-0">Drop files or <strong className="text-[#E87A2E]">browse</strong></p>
                <input type="file" id={`fu${sub.id}`} multiple className="hidden" onChange={handleUpload} />
              </label>
              
              {(uploads[sub.id] || []).map((f, i) => (
                <div key={i} className="flex items-center gap-[0.25rem] p-[0.25rem_0.45rem] bg-[#E8F5E9] rounded-[5px] text-[0.68rem] font-medium text-[#4CAF50] mb-[0.15rem] w-max">
          ...
          <div className="flex flex-wrap items-center justify-between py-[1rem] mt-[0.6rem] border-t border-[#E5DDD4] gap-[0.5rem]">
            <button 
              className={`px-[1.1rem] py-[0.5rem] rounded-[9px] border text-[0.78rem] font-semibold flex items-center gap-[0.3rem] transition-all cursor-pointer ${
                isDone ? 'bg-[#4CAF50] text-white border-[#4CAF50] hover:bg-[#388E3C]' : 'bg-[#F9F5F0] text-[#6B6D7B] border-[#E5DDD4] hover:border-[#4CAF50] hover:text-[#4CAF50]'
              }`}
            ...
            {nextSub && (
              <button 
                className={`px-[1.1rem] py-[0.5rem] rounded-[9px] border-none text-[0.78rem] font-semibold flex items-center gap-[0.3rem] transition-all ${
                  canNext ? 'bg-[#E87A2E] text-white cursor-pointer hover:bg-[#D06A20]' : 'bg-[#E87A2E] text-white opacity-35 cursor-default pointer-events-none'
                }`}
                onClick={() => canNext && setCurS(curS + 1)}
              >
                Next: {nextSub.title.length > 20 ? nextSub.title.substring(0, 20) + '…' : nextSub.title}
...
const WeekTabs: React.FC<{ w: typeof WK; curW: number; setCurW: (i: number) => void }> = ({ w, curW, setCurW }) => (
    ...
    {w.map((wk, i) => (
      <button 
        ...
        className={`px-[1rem] py-[0.45rem] rounded-full border-[1.5px] font-semibold text-[0.72rem] transition-all font-inherit ${
          i === curW ? 'bg-[#E87A2E] text-white border-[#E87A2E]' : 
          !wk.ul ? 'bg-white border-[#E5DDD4] text-[#6B6D7B] opacity-40 cursor-default hover:border-[#E5DDD4] hover:text-[#6B6D7B]' : 
