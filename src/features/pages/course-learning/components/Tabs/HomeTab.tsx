...
export const HomeTab: React.FC<HomeTabProps> = ({ goToCourseContent, done }) => {
  return (
    <div className="max-w-[860px] mx-auto p-[1rem] md:p-[1.3rem_1.8rem_3rem]">
      <div className="bg-white rounded-[14px] border border-[#E5DDD4] p-[0.8rem] mb-[1rem] flex items-center gap-[0.7rem]">
        ...
        <div>
          <h4 className="text-[0.8rem] font-bold text-[#2B2D42] m-0">5-Day Streak!</h4>
      ...
      <div className="bg-white rounded-[14px] border border-[#E5DDD4] overflow-hidden mb-[1rem]">
        <div className="p-[1rem_1.2rem] text-[0.82rem] text-[#6B6D7B] leading-[1.65]">
          <strong className="text-[#2B2D42]">Welcome!</strong> Master prompt engineering, multimodal AI, Python automation, and responsible AI practices across 4 intensive weeks. Each sub-topic must be completed before the next unlocks — building deep, sequential mastery.
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-[0.5rem] my-[1rem]">
        {[
          ...
          <div key={idx} className="bg-white rounded-[8px] border border-[#E5DDD4] p-[0.7rem] text-center">
            <div className="font-['DM_Serif_Display'] text-[1.15rem] text-[#E87A2E]">{n}</div>
      ...
      <div className="bg-white rounded-[14px] border border-[#E5DDD4] overflow-hidden mb-[1rem]">
        <div className="p-[1rem_1.2rem] border-b border-[#E5DDD4] flex items-center justify-between">
          <h2 className="font-['DM_Serif_Display'] text-[0.95rem] text-[#2B2D42] m-0">Weekly Timeline</h2>
        </div>
            ...
            return (
              ...
              >
                <div 
                  ...
                  style={{ backgroundColor: isWeekComplete ? '#4CAF50' : w.color }}
                ></div>
                <div className="flex-1">
                  <div className="text-[0.76rem] font-semibold text-[#2B2D42]">{w.t}</div>
                  <div className="text-[0.64rem] text-[#6B6D7B]">
                    {doneCt}/{w.subs.length} completed · {w.ul ? 'Available' : 'Locked'}
                  </div>
                </div>
                <div 
                  className={`text-[0.6rem] font-semibold px-[0.45rem] py-[0.18rem] rounded-full ${
                    w.ul ? 'bg-[#e87a2e1f] text-[#E87A2E]' : 'bg-[#F9F5F0] text-[#9597A6]'
                  }`}
                >
                  {w.short}
                </div>
