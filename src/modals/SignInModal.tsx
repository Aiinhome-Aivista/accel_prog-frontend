  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (response.status === "success") {
        ...
        showError(
          "Sign-in error",
          response.message || "Backend verification failed.",
        );
      }
    } catch (error: any) {
      if (error.code === "auth/configuration-not-found") {
        console.error(
          "Firebase Configuration Error: Auth not enabled or project/domain mismatch.",
        );
        showError(
          "Sign-in error",
          "Sign-in configuration not found. Please verify Firebase project settings.",
        );
